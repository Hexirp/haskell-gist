...
module Main where
  ...
  emptySource :: Source (Cio r r) a
  emptySource = Source $ pureCio Done
  ...
  runCoroutine m = Source $ resetCio $ (liftIOCio $ newIORef emptySource) `bindCio` \ref -> m ref `bindCio` \_ -> (liftIOCio $ readIORef ref) `bindCio` unSource
