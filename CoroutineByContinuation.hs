...
module Main where
  ...
  yield :: a -> IORef (Source (Cio _ _) a) -> Cio r o (Source (Cio _ _) a)
  yield x ref = joinCio $ fmapCio (\res -> liftIOCio $ writeIORef ref $ Source $ pureCio res) $ shiftCio $ \k -> pure $ More x $ Source $ resetCio $ k Done
